...
# CORS — allow all origins for Angular app on Netlify
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
async def predict(file: UploadFile = File(...), user=Depends(validate_azure_token)):
    ...
    2. Upload to Databricks staging table
    ...
    row_count = len(df)
    site_count = df["location_nme"].nunique()

    # ── 2. Upload to Databricks staging table ──
    # Write CSV to DBFS, then load into a staging table via notebook
    upload_id = f"upload_{int(time.time())}"
    dbfs_path = f"/FileStore/gev_ehs/uploads/{upload_id}.csv"

    try:
        # Upload CSV bytes to DBFS
        client.dbfs.put(dbfs_path, io.BytesIO(content), overwrite=True)
        print(f"Uploaded {row_count} rows to dbfs:{dbfs_path}")
    except Exception as e:
        raise HTTPException(500, f"Failed to upload to DBFS: {e}")
    ...
    try:
        run = client.jobs.submit(
            run_name=f"gev_ehs_inference_{upload_id}",
            tasks=[
                SubmitTask(
                    task_key="inference",
                    existing_cluster_id=cluster_id,
                    notebook_task=NotebookTask(
                        notebook_path=INFERENCE_NOTEBOOK,
                        base_parameters={
                            "upload_id": upload_id,
                            "dbfs_csv_path": dbfs_path,
                        },
                    ),
                )
            ],
    ...
    while elapsed < max_wait:
        ...
        if life_cycle in ("TERMINATED", "SKIPPED", "INTERNAL_ERROR"):
            if result_state == "SUCCESS":
                ...
                error_msg = state.state_message or "Unknown error"
                raise HTTPException(500, f"Job failed: {error_msg}")

        time.sleep(poll_interval)
        elapsed += poll_interval

    if elapsed >= max_wait:
        raise HTTPException(504, "Job timed out after 10 minutes")

    # ── 5. Fetch results from the output table ──
    try:
        results = fetch_prediction_results(client, upload_id)
